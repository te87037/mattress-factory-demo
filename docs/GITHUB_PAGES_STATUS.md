# GitHub Pages 部署狀態

官網與 Sanity Studio 建置：success

GitHub Pages 發布：abandoned

本次部署電話：037-742-5180

Workflow Run ID：31116721762

官網：https://te87037.github.io/mattress-factory-demo/

內容後台（Sanity Studio）：https://te87037.github.io/mattress-factory-demo/studio/

手動部署：https://github.com/te87037/mattress-factory-demo/actions/workflows/deploy-pages.yml

老闆交接手冊：https://github.com/te87037/mattress-factory-demo/blob/main/docs/handoff/SKILL.md

PageCMS／Pages CMS：已停用，不再使用 app.pagescms.org、cms-preview 或 sanity-cms。
